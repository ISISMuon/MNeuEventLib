...
impl WiMDAFile {
    pub fn new(dataset: &NexusData, filters: &Filters, results: &Histogram) -> Result<WiMDAFile> {
        let unfiltered_frames = dataset.periods.size() as u32;
        // raw frames is all frames that have not been filtered,
        // good frames is all frames that have not been filtered or vetoed
        let raw_frames = results.n_frames.iter().sum::<u32>();
        let good_frames = results.n_good_frames.iter().sum::<u32>();
        ...
        let instrument = Instrument::new(results, dataset.n_spec);

        let periods = Periods::new(results);

        let selog = get_all_sample_logs(dataset, filters)?;
...
impl SaveFile for WiMDAFile {
    fn save_file(&self, filename: String, input_file: &File) -> Result<()> {
        ...
        self.save(&hist_data, &event_data)?;
        println!("Saved to output file {filename}");
        Ok(())
    }
}

impl Save for WiMDAFile {
    fn save(&self, hist_data: &Group, event_data: &Group) -> Result<()> {
